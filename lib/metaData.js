...
const getMetadata = async (url, model = null) => {
    ...
    switch (compilerType) {
        ...
        case COMPILER_TYPES.graphql:
            try {
                ...
                responseFile = printSchema(lexicographicSortSchema(generateSchema4(services)));
            } catch (error) {
    ...
    return {
        contentType: compilerType === "graphql" ? "text/plain" : `application/${compilerType === "edmx" ? "xml" : "json"}`,
