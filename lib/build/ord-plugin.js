...
const ord = require("../core/ord");
const { BUILD_DEFAULT_PATH, ORD_DOCUMENT_FILE_NAME } = require("../common/constants");
...
module.exports = class OrdBuildPlugin extends cds.build.Plugin {
    ...
    async build() {
        ...
        return this.model()
            .then((model) => ({ model, document: this._generateDocument(model) }))
            .then(({ model, document }) =>
                Promise.all([
