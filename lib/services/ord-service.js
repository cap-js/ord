...
const { withSpan, counters } = require("../telemetry");
    ...
    return withSpan(
        ...
        async (span) => {
            try {
                ...
                if (span) span.setAttribute("http.response.header.content-type", contentType);
                counters.metadataRequests.add(1, {
                    "http.route": req.route?.path,
...
class OpenResourceDiscoveryService extends cds.ApplicationService {
    async init() {
        ...
        cds.app.get(`${this.path}`, async (req, res) => {
            return withSpan(
                "ord.well_known.serve",
                {
                    "http.route": this.path,
                    "http.request.method": req.method,
                },
                async () => {
                    const toggles = cds.env.requires.toggles;
        ...
        cds.app.get(`/ord/v1/documents/ord-document`, [authMiddleware, validationMiddleware], async (req, res) => {
            return withSpan(
                "ord.document.serve",
                {
                    "http.route": "/ord/v1/documents/ord-document",
                    "http.request.method": req.method,
                    "ord.perspective": req.query.perspective || DOCUMENT_PERSPECTIVES.SystemVersion,
                },
                async (span) => {
                    ...
                    if (isNaN(part) || part < 0 || part >= slices.length) {
                        if (span) span.setAttribute("http.response.status_code", 400);
                        return res.status(400).send(`Required query parameter 'part' is invalid`);
                    }

                    if (span) {
                        span.setAttribute("ord.document.part", Number(part));
                        span.setAttribute("ord.document.parts.total", slices.length);
                    }
                    counters.metadataRequests.add(1, {
                        "http.route": "/ord/v1/documents/ord-document",
                        "http.response.status_code": 200,
