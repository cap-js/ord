...
const { localize } = require("../common/utils.js");
...
const metadataResponseHandler = async (req, res) => {
    try {
        ...
        return res.status(200).contentType(contentType).send(localize(model, response));
    } catch (error) {
...
const resolveCdsModel = async (perspective, tenant) => {
    const dynamic = tenant && perspective === DOCUMENT_PERSPECTIVES.SystemInstance;
    const toggles = dynamic
        ? await OpenResourceDiscoveryService.resolveFeatureToggles(tenant)
        : await OpenResourceDiscoveryService.resolveDefaultFeatureToggles();

    if (!dynamic && toggles.some((toggle) => "*" === toggle)) {
        Logger.info("Retrieving static CDS model...");
        return cds.model;
    }

    Logger.info(`Retrieving dynamic CDS model for tenant '${tenant ?? "system"}'...`);
...
class OpenResourceDiscoveryService extends cds.ApplicationService {
    async init() {
        ...
        cds.app.get(`${this.path}`, async (req, res) => {
            ...
            const tenant = req.headers[LOCAL_TENANT_ID_HEADER_KEY];
            const extensions = [
                ...Array.from(Object.values(this.extensions)),
    ...
    static async resolveFeatureToggles(tenant) {
        return ["*"];
    }

    static async resolveDefaultFeatureToggles() {
        return ["*"];
    }
}
