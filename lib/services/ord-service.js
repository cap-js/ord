...
class OpenResourceDiscoveryService extends cds.ApplicationService {
    async init() {
            ...
            return res
                ...
                .send(
                    defaults.baseTemplate(
                        authConfig,
                        resolveCdsModel(DOCUMENT_PERSPECTIVES.SystemVersion, tenant),
                        !tenant || (!toggles && !extensibility)
                            ? undefined
                            : resolveCdsModel(DOCUMENT_PERSPECTIVES.SystemInstance, tenant),
                    ),
                );
        });
        ...
        cds.app.get(`/ord/v1/documents/ord-document`, [authMiddleware, validationMiddleware], async (req, res) => {
            ...
            return res
                .status(200)
                .send(split(defaults.adjustForPerspective(ord(model, extensions), perspective))[part]);
        });
