...
const fInitializeGlobal = (csn) => {
    ...
    const fServicesFilter = (srv) => csn.definitions[srv].kind === "service" && !csn.definitions[srv]['@cds.external'];
    ...
    const aServices = aDefinitions.filter((srv) => fServicesFilter(srv));
 ...
 */
const fGetAPIResources = (csn) => global.aServices.map((srv) => fCreateAPIResourceTemplate(srv, csn));
