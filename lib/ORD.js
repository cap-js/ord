...
const fInitializeGlobal = () => {
    ...
    // const fEventFilter = (key) => fGetEntityDefinition(key).kind === "event" && !fGetEntityDefinition(key)['@cds.external'];
    // const fServicesFilter = (srv) => !srv['@cds.external'];
    ...
    const aEvents = Object.values(cds.model.definitions).filter((ele) => ele.kind === "event" && !ele['@cds.external'])
    const aServices = Object.values(cds.model.definitions).filter((ele) => ele.kind === "service" && !ele['@cds.external'] )
    const aODMEntity = Object.values(cds.model.definitions).filter((ele) => ele.kind === "entity" && !ele['@cds.external'] && ele["@ODM.entityName"]).map((entity) => fCreateEntityTypeTemplate(entity));
 ...
 */
const fGetAPIResources = () => global.aServices.map((srv) => fCreateAPIResourceTemplate(srv.name));
 ...
 */
const fGetEventResources = () => global.aEvents.map((srv) => fCreateEventResourceTemplate(srv.name));
 ...
 */
const fGetIntegrationDependencies = () => global.aIntegrationDependency.map((srv) => fCreateIntegrationDependencyTemplate(cds.model.namespace,srv.name));
...
module.exports = () => {
    try {
        ...
        const oReturn = {
            ...
            integrationDependencies: fGetIntegrationDependencies(),
        };
        return oReturn;
    } catch (error) {
        console.log(error);
