            ...
            case CDS_ELEMENT_KIND.service:
                if (!keyDefinition["@cds.external"]) {
                    ...
                    if (keyDefinition[DATA_PRODUCT_ANNOTATION] === DATA_PRODUCT_TYPE.primary) {
                        ...
                        entityTypeTargets.push(createEntityTypeMappingsItemTemplate())
                    }
                }
                break;
...
const _getEntityTypes = (appConfig, packageIds) => {
    ...
    const importEntityTypes = appConfig.dataProducts.map(dp => [
        {
            ordId: "sap.epd:entityType:PLMProduct:v1",
            localId: "PLMProduct",
            title: dp.name, // TODO
            shortDescription: "Short description of Product",
            description: "Description for Product",
            version: "1.0.0",
            lastUpdate: "2024-12-06T13:22:45+01:00",
            visibility: "public",
            partOfPackage: "sap.epd:package:sap-epd-entityTypes:v1",
          ...
          {
            ordId: "sap.epd:entityType:PLMProductRoot:v1",
            localId: "PLMProduct",
            title: dp.name, // TODO
            shortDescription: "Short description of Product",
            description: "Description for Product",
    ...
    const exposedEntityTypes = internalEntityTypes.map(entity => createEntityTypeTemplate(appConfig, packageIds, entity))
    return [...importEntityTypes, ...exposedEntityTypes];
};
...
const _getAPIResources = (csn, appConfig, packageIds, accessStrategies) => {
    ...
    return appConfig.serviceNames
        ...
        .flatMap((serviceName) => createAPIResourceTemplate(serviceName, csn.definitions[serviceName], appConfig, packageIds, accessStrategies))
};
...
const _getEventResources = (csn, appConfig, packageIds, accessStrategies) => {
    ...
    return appConfig.serviceNames
        ...
        .flatMap((serviceName) => createEventResourceTemplate(serviceName, csn.definitions[serviceName], appConfig, packageIds, accessStrategies));
};
