...
const initializeAppConfig = (csn, modelPath = process.cwd()) => {
    ...
    if (cds.utils.exists(packageJsonPath)) {
        ...
        throw new Error(`package.json not found in the directory: ${packageJsonPath}. Ensure you run the command from the correct context.`);
    }
...
module.exports = (csn, modelPath = process.cwd()) => {
    if (typeof modelPath === 'object') {
        modelPath = process.cwd();
    }
    ...
    const appConfig = initializeAppConfig(linkedCsn, modelPath);
