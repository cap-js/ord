...
function _isValidService(key, definition) {
    ...
    try {
        isExternalService = Object.keys(cds).includes("requires") && cds.requires ? Object.keys(cds.requires).includes(key) : false;
    } catch (error) {
        // cds.requires is not accessible, treat as not external
...
const _getIntegrationDependencies = (csn, appConfig, packageIds, accessStrategies) => {
    const integrationDependencies = [];
    
    // Check if cds.requires exists and contains external services
    // Use try-catch to handle cases where cds.requires is not defined or accessible
    let externalServices = {};
    try {
        externalServices = cds.requires || {};
    } catch (error) {
        // cds.requires is not accessible, return empty array
        return integrationDependencies;
    }
    
    // For each service in cds.requires, create an integration dependency
    for (const serviceName of Object.keys(externalServices)) {
        const serviceConfig = externalServices[serviceName];
        const serviceDefinition = csn.definitions[serviceName];
        
        // Only create integration dependency for external services
        if (serviceConfig.kind === "external" && serviceDefinition) {
            const intDep = createIntegrationDependencyTemplate(
                serviceName,
                serviceDefinition,
                appConfig,
                packageIds,
...
function _filterUnusedPackages(ordDocument) {
    ...
    ordDocument.integrationDependencies?.forEach((intDep) => usedPackageIds?.add(intDep.partOfPackage));
...
module.exports = (csn) => {
    ...
    // Add integration dependencies for external services
    const integrationDependencies = _getIntegrationDependencies(linkedCsn, appConfig, packageIds, accessStrategies);
