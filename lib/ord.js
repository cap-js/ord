...
const _getProducts = (appConfig) => {
    ...
    if (appConfig.env?.products) {
        _.assign(productsObj[0], appConfig.env.products[0]);
    }
    appConfig.env.products = productsObj;
    return productsObj;
}
...
const _getPackages = (policyLevel, appConfig) => {
    return appConfig.env?.packages || appConfig.events.length
        ? defaults.packages(appConfig.appName, policyLevel, appConfig.ordNamespace, appConfig.env?.products[0]?.ordId,
            appConfig.env?.packages[0]?.vendor)
        : defaults
