...
function _handleProduct(appConfig) {
    if (appConfig.env?.existingProductORDId) {
        const existingProduct = appConfig.products.find(product => product.ordId === appConfig.env.existingProductORDId);
        if (existingProduct) {
            return existingProduct;
        } else {
            Logger.error(`The provided existingProductORDId ${appConfig.env.existingProductORDId} does not exist.`);
        }
    } else {
        return _getProducts(appConfig)[0];
    }
}
...
function createDefaultORDDocument(linkedCsn, appConfig) {
    ...
    if (appConfig.env?.existingProductORDId) {
        ordDocument.products = _getProducts(appConfig);
    }
