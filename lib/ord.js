...
const { getAuthConfigSync, detectAuthConfig } = require("./auth/authentication");
...
module.exports = (csn) => {
    ...
    // Get access strategies - use runtime config if available, otherwise detect build-time config
    let accessStrategies;
    try {
        // Try to get runtime configuration (available when service is running)
        accessStrategies = getAuthConfigSync().accessStrategies;
    } catch (error) {
        // Build-time: authentication config not initialized, use configuration detection
        Logger.info("Build time: Using configuration detection for access strategies");
        const buildTimeConfig = detectAuthConfig();
        
        if (buildTimeConfig.error) {
            Logger.error("Build time configuration error:", buildTimeConfig.error);
            // Use default open strategy as fallback
            accessStrategies = [{ type: "open" }];
        } else {
            accessStrategies = buildTimeConfig.accessStrategies;
        }
    }
