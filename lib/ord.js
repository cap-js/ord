...
const _getGroups = (csn, appConfig) => {
    return appConfig.serviceNames
        .flatMap((srv) => createGroupsTemplateForService(srv, csn.definitions[srv], appConfig))
...
const _getAPIResources = (csn, appConfig, packageIds) => {
    return appConfig.serviceNames
        .flatMap((srv) => createAPIResourceTemplate(srv, csn.definitions[srv], appConfig, packageIds) || [])
        .filter((resource) => !!resource);
};
...
module.exports = (csn) => {
    ...
    ordDocument = {
        ...ordDocument,
        apiResources: _getAPIResources(linkedCsn, appConfig, packageIds),
    };

    if (_getEventResources(linkedCsn, appConfig, packageIds).length > 0) {
        ordDocument.eventResources = _getEventResources(
            linkedCsn,
            appConfig,
            packageIds
        );
