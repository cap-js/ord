...
const { isEventBrokerReady, getEventBrokerOrdInfo } = require("./eventBrokerAdapter");
...
/**
 * Add Event Broker Integration Dependencies if event-broker is configured.
 * This detects @cap-js/event-broker messaging services and generates
 * ORD Integration Dependencies documenting consumed event types.
 *
 * @param {Object} ordDocument - The ORD document to add integration dependencies to
 * @param {Object} appConfig - The application configuration
 * @param {Object} csn - The CDS CSN model (for model-based event detection)
 * @returns {void}
 */
const _addEventBrokerIntegrationDependencies = (ordDocument, appConfig, csn) => {
    if (!isEventBrokerReady()) {
        return;
    }

    try {
        const eventBrokerInfo = getEventBrokerOrdInfo({
            fallbackNamespace: appConfig.ordNamespace,
            csn, // Pass CSN for model-based event detection
        });

        if (!eventBrokerInfo) {
            Logger.log("Event Broker detected but no ORD info available");
            return;
        }

        if (!eventBrokerInfo.hasEvents) {
            Logger.log("Event Broker detected but no consumed events found");
            return;
        }

        // Get package ordIds for visibility-based package selection
        const packageIds = ordDocument.packages?.map((p) => p.ordId) || [];
        const packageVersion = ordDocument.packages?.[0]?.version || "1.0.0";

        if (packageIds.length === 0) {
            Logger.warn("Cannot create Event Broker Integration Dependency: no packages available");
            return;
        }

        const integrationDependency = createIntegrationDependencyTemplate({
            ordNamespace: appConfig.ordNamespace,
            sourceNamespace: eventBrokerInfo.namespace,
            eventTypes: eventBrokerInfo.eventTypes,
            packageIds,
            packageVersion,
        });

        if (integrationDependency) {
            ordDocument.integrationDependencies = ordDocument.integrationDependencies || [];
            ordDocument.integrationDependencies.push(integrationDependency);
            Logger.log(
                `Added Event Broker Integration Dependency with ${eventBrokerInfo.eventTypes.length} event types`,
            );
        }
    } catch (error) {
        Logger.warn("Failed to create Event Broker Integration Dependency:", error.message);
    }
};
...
function _filterUnusedPackages(ordDocument) {
    ...
    ordDocument.integrationDependencies?.forEach((dep) => {
        if (dep && dep.partOfPackage) {
            usedPackageIds.add(dep.partOfPackage);
        }
    });
...
module.exports = (csn) => {
    ...
    // Add Event Broker Integration Dependencies if configured
    // Pass linkedCsn to enable model-based event detection via @ORD.Extensions.consumedEvent
    _addEventBrokerIntegrationDependencies(ordDocument, appConfig, linkedCsn);
