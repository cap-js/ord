...
const fGetEventResources = (csn, global,packageIds) => {
    ...
    const services = []
    for (const srv of global.aServices) {
        const hasEvents = global.aEvents.find((eventName) => eventName.startsWith(srv));
        if (hasEvents) {
            services.push(srv)
        }
    }

    return services.map((srv) => fCreateEventResourceTemplate(srv, csn.definitions[srv], global, packageIds))
};
