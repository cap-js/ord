...
const { withSpanSync, counters, histograms } = require("./telemetry");
...
module.exports = (csn, extensions) => {
    ...
    return withSpanSync(
        ...
        (span) => {
            ...
            // Resource-level attributes — useful for filtering in the backend.
            const attrs = {
            ...
            histograms.documentGenerationDuration.record(durationMs, {
                "ord.namespace": appConfig?.env?.namespace,
            });
            counters.documentsGenerated.add(1, {
                "ord.namespace": appConfig?.env?.namespace,
            });
