...
const initializeAppConfig = (csn) => {
        ...
        switch (keyDefinition.kind) {
            ...
            case CDS_ELEMENT_KIND.entity:
                if (!key.includes(".texts")) {
                    ...
                    if (keyDefinition[ORD_ODM_ENTITY_NAME_ANNOTATION]) {
    ...
    return {
        ...
        serviceNames,
        externalServices,
        odmEntities: _.uniqBy(odmEntities, CONTENT_MERGE_KEY),
        ordNamespace,
...
const _getIntegrationDependencies = (csn, appConfig, packageIds) => {
    return appConfig.externalServices
        .filter((serviceName) => appConfig.events.some((eventName) => eventName.startsWith(serviceName)))
        .map((serviceName) => createIntegrationDependencyTemplate(serviceName, csn.definitions[serviceName], appConfig, packageIds) || []);
}
...
module.exports = (csn) => {
    ...
    ordDocument.integrationDependencies = _getIntegrationDependencies(linkedCsn, appConfig, packageIds);
