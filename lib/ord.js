...
const _getIntegrationDependencies = (linkedCsn, appConfig) => {
    return !appConfig.externalServiceNames?.length ? [] : [createIntegrationDependency(linkedCsn, appConfig)];
};
...
const _createDefaultORDDocument = (linkedCsn, appConfig) => {
    ...
    const apiResources = _getAPIResources(linkedCsn, appConfig, packageIds);
    const integrationDependencies = _getIntegrationDependencies(linkedCsn, appConfig);
