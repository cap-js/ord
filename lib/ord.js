...
const initializeAppConfig = (csn) => {
    ...
    const { apiResourceNames, apiEndpoints, eventNames, entityTypeTargets } = _triageCsnDefinitions(csn);
...
const _getGroups = (csn, appConfig) => {
    return appConfig.apiResourceNames
        .flatMap((apiResourceName) =>
            createGroupsTemplateForService(apiResourceName, csn.definitions[apiResourceName], appConfig),
        )
...
function createDefaultORDDocument(linkedCsn, appConfig) {
    ...
    if (_getGroups(linkedCsn, appConfig).length) {
