...
const _getEntityTypes = (appConfig, packageIds) => {
    if (appConfig.entityTypeTargets?.length === 0 ||
        appConfig.entityTypeTargets.filter(m => m[ENTITY_RELATIONSHIP_ANNOTATION]).length === 0) return [];

    const internalEntityTypes = appConfig.entityTypeTargets.filter(m => m[ENTITY_RELATIONSHIP_ANNOTATION]);
    return internalEntityTypes.map((entity) => {
        return createEntityTypeTemplate(appConfig, packageIds, entity);
    });
};
...
module.exports = (csn) => {
    ...
    ordDocument.entityTypes = _getEntityTypes(appConfig, packageIds);
