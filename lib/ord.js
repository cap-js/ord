...
const _getEntityTypes = (appConfig, packageIds) => {
    if (!appConfig.entityTypeTargets || appConfig.entityTypeTargets.length === 0) return [];
...
const _getDataProducts = (appConfig, packageIds) => {
    return Array.isArray(appConfig.dataProducts)
    ? appConfig.dataProducts.flatMap((dataProduct) =>
        createDataProductTemplate(dataProduct.localId, dataProduct, appConfig, packageIds)
    )
    : [];
};
...
function createDefaultORDDocument(linkedCsn, appConfig) {
    ...
    const dataProducts = _getDataProducts(appConfig, extractPackageIds(ordDocument));
...
function _getPackageID(namespace, packageIds = [], resourceType, visibility = RESOURCE_VISIBILITY.public) {
    if (!Array.isArray(packageIds) || packageIds.length === 0) {
        return `${namespace}:package:${resourceType}-${visibility}:v1`;
    }

    const packageId = packageIds.find(id => id.includes(`-${resourceType}-${visibility}`));
    return packageId || `${namespace}:package:${resourceType}-${visibility}:v1`;
}
...
function _filterUnusedPackages(ordDocument) {
    ...
    ordDocument.entityTypes?.forEach(et => usedPackageIds.add(et.partOfPackage));
