...
module.exports = class OrdBuildPlugin extends cds.build.Plugin {
    ...
    async build() {
        const extensions = [getCustomORDContent(cds?.env?.["ord"]?.customOrdContentFile)];
        ...
        return this.model()
            .then((model) =>
                Promise.all([
                    this._generateResourcesFiles(model), // No extensions from custom ORD JSON here
                    this.write(this._postProcess(ord(model, extensions))).to(ORD_DOCUMENT_FILE_NAME),
                ]),
            )
            .catch((error) => {
    ...
    _generateResourcesFiles(model) {
        const document = ord(model); // Do not generate resources for resources coming from custom ORD content
        const progressBar = this._createProgressBar();
        const tasks = this._extractCompileTasks([...(document.apiResources ?? []), ...(document.eventResources ?? [])]);
        ...
        return Promise.all(
            tasks.map(({ url, ordId }) =>
                pool.run({ url }).then((response) =>
                    this.write(localize(model, response))
