...
module.exports = class OrdBuildPlugin extends cds_dk.build.Plugin {
    ...
    postProcessWithRelativePath(ordDocument) {
        const updateResourceUrls = (resources) => {
            for (const resource of resources || []) {
                if (resource.resourceDefinitions) {
                    for (const resourceDefinition of resource.resourceDefinitions) {
                        const url = resourceDefinition.url;
                        resourceDefinition.url = path.join(
                            BUILD_DEFAULT_PATH,
                            url.split("/ord/v1").pop()
                        );
                    }
                }
            }
        ...
        updateResourceUrls(ordDocument.apiResources);
        updateResourceUrls(ordDocument.eventResources);
    ...
    async build() {
        ...
        const postProcessedOrdDocument = this.postProcessWithRelativePath(ordDocument);
