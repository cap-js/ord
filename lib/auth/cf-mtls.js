...
function isXfccProxyVerified(req) {
    const { CF_MTLS_HEADERS } = require("../common/constants");
...
function extractCertHeaders(req, headerNames) {
    const { CF_MTLS_ERROR_REASON } = require("../common/constants");
...
function handleCfMtlsAuthentication(req, res, authConfig, Logger) {
    const { AUTHENTICATION_TYPE, CF_MTLS_ERROR_REASON, AUTH_STRINGS } = require("../common/constants");
