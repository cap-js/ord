...
async function createCfMtlsConfig(cds, Logger) {
    ...
    if (!cfInstanceGuid && !vcapApplication) {
        Logger.error(
            "CF mTLS authentication requires Cloud Foundry environment. " +
                "Authentication will fail outside of SAP BTP Cloud Foundry.",
        );
    } else if (!cfInstanceGuid) {
        Logger.warn("CF mTLS authentication: CF_INSTANCE_GUID not set. Possible non-standard deployment.");
    }
