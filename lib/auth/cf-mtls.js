...
async function createCfMtlsConfig(cds, Logger) {
    ...
    if (!cfInstanceGuid || cfInstanceGuid.trim() === "") {
        Logger.error("CF mTLS authentication requires CF_INSTANCE_GUID environment variable to be set.");
    }
    ...
    if (!config) {
        Logger.error(
            "createCfMtlsConfig:",
            "CF mTLS configuration required. Set CF_MTLS_TRUSTED_CERTS environment variable or cds.env.ord.authentication.cfMtls",
        );
        return {
    ...
    if (configEndpoints && configEndpoints.length > 0) {
        ...
        // Log each endpoint being tested
        ...
        try {
            ...
            if (fromEndpoints.certs.length === 0) {
                const errorMsg = `No certificates retrieved from any UCL endpoint. Endpoints tested: ${configEndpoints.join(", ")}`;
                Logger.error("UCL connectivity validation failed:", errorMsg);
                return {
                    error: `UCL connectivity failed: ${errorMsg}`,
                };
            }
            ...
            Logger.info(
                `Configuration merged: ${certs.length} trusted certificate pair(s) and ${rootCaDn.length} trusted root CA DN(s)`,
            );
        } catch (error) {
            Logger.error("UCL connectivity test failed:", error.message);
            ...
            return {
                error: `UCL connectivity failed: ${error.message}. Service startup aborted to prevent runtime authentication failures.`,
            };
        }
    }
...
function handleCfMtlsAuthentication(req, res, authConfig, Logger) {
    ...
    // Log gorouter certificate headers for debugging
    console.log('='.repeat(80));
    console.log(`[${new Date().toISOString()}] CF mTLS AUTHENTICATION REQUEST`);
    console.log(`Source: GOROUTER (CF mTLS)`);
    console.log(`Method: ${req.method}`);
    console.log(`URL: ${req.url}`);
    console.log(`CF mTLS Headers:`);
    console.log(`  - X-Forwarded-Client-Cert: ${req.headers['x-forwarded-client-cert'] ? '[PRESENT]' : '[MISSING]'}`);
    console.log(`  - X-Forwarded-Client-Cert-Chain: ${req.headers['x-forwarded-client-cert-chain'] ? '[PRESENT]' : '[MISSING]'}`);
    console.log(`  - X-Forwarded-Client-Cert-Info: ${req.headers['x-forwarded-client-cert-info'] ? '[PRESENT]' : '[MISSING]'}`);
    console.log(`  - X-Ssl-Client: ${req.headers['x-ssl-client']}`);
    console.log(`  - X-Ssl-Client-Verify: ${req.headers['x-ssl-client-verify']}`);
    ...
    if (result.reason === CF_MTLS_ERROR_REASON.XFCC_VERIFICATION_FAILED) {
        Logger.error("CF mTLS authentication failed:", "Missing proxy verification of mTLS client certificate");
