 ...
 * 2. Multiple Authentication: Basic, CF/DwC mTLS, and other methods can coexist
 ...
 * - dwc-mtls: DwC mTLS authentication
 *
...
const { createDwcMtlsConfig, handleDwcMtlsAuthentication } = require("./dwc-mtls");
const {
    ORD_ACCESS_STRATEGY,
...
/**
 * Lazy-loads and initializes the DwC mTLS validator.
 ...
 * @returns {Promise<Function>} The DwC mTLS validator function
 * @throws {Error} If DwC mTLS initialization fails
 */
async function ensureDwcMtlsValidator(authConfig) {
    // Already initialized
    if (authConfig.mtlsValidator) {
        return authConfig.mtlsValidator;
    }

    // Initialization in progress - wait for it
    if (authConfig._mtlsInitPromise) {
        await authConfig._mtlsInitPromise;
        return authConfig.mtlsValidator;
    }

    // Start initialization
    Logger.info("Initializing DwC mTLS validator (lazy loading)...");

    authConfig._mtlsInitPromise = (async () => {
        try {
            const { mtlsValidator, error } = await createDwcMtlsConfig(cds, Logger);

            if (error) {
                throw new Error(error);
            }

            authConfig.mtlsValidator = mtlsValidator;
            Logger.info("DwC mTLS validator initialized successfully");
        } catch (error) {
            // Clean up on failure so retry is possible
            authConfig._mtlsInitPromise = null;
            throw error;
        }
    })();
...
async function ensureCfMtlsValidator(authConfig) {
    ...
    authConfig._mtlsInitPromise = (async () => {
        try {
            const { mtlsValidator, error } = await createCfMtlsConfig(cds, Logger);
 ...
 * 1. Environment variables (BASIC_AUTH, CF_MTLS_TRUSTED_CERTS, DWC_MTLS_TRUSTED_CERTS) - for production deployments
 * 2. .cdsrc.json settings (cds.env.ord.authentication.basic, cds.env.ord.authentication.cfMtls, cds.env.ord.authentication.dwcMtls) - for development and testing
 *
 ...
 * - If cds.env.ord.authentication.dwcMtls exists → DwC mTLS authentication enabled
 ...
 *
 * Note: CF/DwC mTLS validator is lazily initialized on first use to avoid blocking
...
function createAuthConfig() {
    ...
    try {
        ...
        // Detect DwC mTLS authentication by checking for dwcMtls config
        if (process.env.DWC_MTLS_TRUSTED_CERTS || ordAuth.dwcMtls) {
            const { accessStrategies } = JSON.parse(
                process.env.DWC_MTLS_TRUSTED_CERTS ??
                    JSON.stringify(typeof ordAuth.dwcMtls === "object" ? ordAuth.dwcMtls : {}),
            );

            // Mark for lazy initialization - validator will be loaded on first use
            authConfig.mtlsValidator = null;
            authConfig._mtlsInitPromise = null;
...
/**
 * Authentication strategy handler for DwC mTLS authentication
 */
async function dwcMtlsAuthStrategy(req, res, authConfig) {
    // Check if request has mTLS indicators
    const hasMtlsHeaders = Object.values(DWC_MTLS_HEADERS).some((header) => req.headers[header.toLowerCase()]);
    ...
    try {
        // Lazy-load validator on first mTLS request
        await ensureDwcMtlsValidator(authConfig);
        ...
        const result = handleDwcMtlsAuthentication(req, mockRes, authConfig, Logger);

        // If handleDwcMtlsAuthentication sent a response, we need to send it
        if (!result.success && capturedStatus && capturedMessage) {
            res.status(capturedStatus).send(capturedMessage);
            return { success: false, handled: true, responseSent: true };
        }
        ...
        Logger.error("DwC mTLS initialization failed:", error.message);
        return { success: false, handled: true, error: "Authentication configuration error" };
    }
}

/**
 * Authentication strategy handler for CF / DwC mTLS authentication
 */
async function mtlsAuthStrategy(req, res, authConfig) {
    const ordAuth = cds.env.ord?.authentication || {};

    if (process.env.DWC_MTLS_TRUSTED_CERTS || ordAuth.dwcMtls) {
        return dwcMtlsAuthStrategy(req, res, authConfig);
    }

    if (process.env.CF_MTLS_TRUSTED_CERTS || ordAuth.cfMtls) {
        return cfMtlsAuthStrategy(req, res, authConfig);
    }
...
const AUTH_STRATEGIES = {
    ...
    [ORD_ACCESS_STRATEGY.CmpMtls]: mtlsAuthStrategy,
    [ORD_ACCESS_STRATEGY.BahMtls]: mtlsAuthStrategy,
};
    ...
    return async function authenticate(req, res, next) {
        ...
        for (const strategy of authConfig.accessStrategies) {
            ...
            try {
                ...
                // If the strategy already sent a response (e.g., CF/DwC mTLS specific error codes)
