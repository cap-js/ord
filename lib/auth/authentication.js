...
/**
 * Get ORD access strategies for the current authentication configuration.
 * This is a safe method that always returns a valid array of access strategies,
 * falling back to 'open' if configuration is invalid.
 *
 * @returns {Array<{type: string}>} Array of access strategy objects
 */
function getOrdAccessStrategies() {
    const authConfig = createAuthConfig();

    if (authConfig.error) {
        Logger.warn("Authentication configuration error, using default 'open' strategy:", authConfig.error);
        return [{ type: AUTHENTICATION_TYPE.Open }];
    }

    return authConfig.accessStrategies;
}

/**
 * Get ORD authentication configuration with validation.
 * Creates configuration synchronously. CF mTLS validator is lazy-loaded on first use.
 *
 * @returns {Object} Authentication configuration
 * @throws {Error} If authentication configuration is invalid
 */
function getOrdAuthConfig() {
    const authConfig = createAuthConfig();
