...
function createAuthConfig() {
    ...
    try {
        ...
        if (process.env.CF_MTLS_TRUSTED_CERTS || ordAuth.cfMtls) {
            ...
            // Validator will be initialized immediately at startup (no lazy loading)
        }
 ...
 * Forces immediate mTLS validator initialization for fail-fast behavior.
 *
 * @returns {Promise<Object>} Authentication configuration
 */
async function getAuthConfig() {
    ...
    // Force immediate CF mTLS validator initialization (no more lazy loading)
    // This ensures UCL connectivity issues are discovered at startup, not at runtime
    if (authConfig.types.includes(AUTHENTICATION_TYPE.CfMtls)) {
        Logger.info("Initializing CF mTLS validator at startup...");
        try {
            Logger.info("Creating CF mTLS validator...");
            ...
            Logger.info("CF mTLS validator created successfully");
        } catch (error) {
            Logger.error("CF mTLS validator initialization failed:", error.message);
            throw new Error("Service startup failed due to mTLS configuration issues: " + error.message);
        }
    }
...
async function authenticate(req, res, next) {
    // Simple fix: If cds.context or authConfig doesn't exist, initialize it
    if (!cds.context?.authConfig) {
        await getAuthConfig();
    }
