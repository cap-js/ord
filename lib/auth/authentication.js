 ...
 * CF mTLS validator is initialized lazily on first use.
 *
...
function getAuthConfig() {
    ...
    cachedAuthConfig = createAuthConfig();
 ...
 * Now just calls the synchronous getAuthConfig().
 *
 * @returns {Object} Authentication configuration
...
async function authenticate(req, res, next) {
    ...
    const isGorouter = req.headers['x-forwarded-client-cert'] || req.headers['x-forwarded-client-cert-chain'] || req.headers['x-forwarded-client-cert-info'];
    const isUCL = req.headers['user-agent']?.includes('UCL') || req.url?.includes('ucl') || req.headers['x-ucl-request'];
