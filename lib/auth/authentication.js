 ...
 *
 * Note: CF mTLS validator is initialized synchronously during configuration creation
 * to ensure proper access strategy generation for ORD documents.
 *
 * @returns {Promise<Object>} Authentication configuration object or default configuration object as a fallback.
 */
async function createAuthConfig() {
    ...
    try {
        ...
        if (process.env.BASIC_AUTH || ordAuth.basic) {
            ...
            authConfig.accessStrategies.push({ type: AUTHENTICATION_TYPE.Basic });
            authConfig.credentials = credentials;
        }

        // Detect CF mTLS authentication by checking for cfMtls config
        if (process.env.CF_MTLS_TRUSTED_CERTS || ordAuth.cfMtls) {
            authConfig.accessStrategies.push({ type: AUTHENTICATION_TYPE.CfMtls });

            // Initialize CF mTLS validator immediately during configuration creation
            const cfMtlsConfig = await createCfMtlsConfig(cds, Logger);

            if (cfMtlsConfig.error) {
                Logger.error("createAuthConfig:", `CF mTLS initialization failed: ${cfMtlsConfig.error}`);
            ...
            Logger.info("createAuthConfig:", "CF mTLS validator initialized successfully");
        }
        ...
        Logger.info("createAuthConfig:", `Configured authentication strategies: ${strategyTypes}`);
        return authConfig;
    } catch (error) {
...
async function authenticate(req, res, next) {
    ...
    // Log request content for gorouter and UCL debugging
    const isGorouter =
    ...
    if (isGorouter || isUCL) {
        ...
        console.log(`[${new Date().toISOString()}] REQUEST CONTENT LOGGING`);
