...
async function fetchMtlsCertInfo(endpoint, timeoutMs) {
    ...
    try {
        console.log(`INFO: Fetching mTLS cert from endpoint: ${endpoint}`);
        ...
        // Log the full raw response for testing purposes
        console.log(`INFO: UCL endpoint raw response from ${endpoint}:`, JSON.stringify(data, null, 2));
        ...
        // Log the parsed certificate information
        console.log(
            `INFO: Parsed certificate from ${endpoint} - Issuer: ${data.certIssuer}, Subject: ${data.certSubject}`,
        );
...
async function fetchMtlsTrustedCertsFromEndpoints(endpoints, Logger, timeoutMs = HTTP_CONFIG.MTLS_TIMEOUT_MS) {
    ...
    const fetchPromises = endpoints.map(async (endpoint) => {
        try {
            ...
            Logger.info(`Successfully fetched mTLS cert info from ${endpoint} - Issuer: ${certInfo.issuer}, Subject: ${certInfo.subject}`);
        } catch (error) {
