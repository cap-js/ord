 ...
 * 
 * This implementation aligns with the provider-server's dynamic endpoint approach
 * as described in PR #183.
 */
 ...
 */
async function fetchMtlsCertInfo(endpoint, timeoutMs, Logger) {
 ...
 * @param {number} [timeoutMs=10000] - Timeout for each request in milliseconds
 * @returns {Promise<{certs: Array<{issuer: string, subject: string}>, rootCaDn: string[]}>}
 */
async function fetchMtlsTrustedCertsFromEndpoints(endpoints, Logger, timeoutMs = DEFAULT_TIMEOUT_MS) {
    ...
    const fetchPromises = endpoints.map(async (endpoint) => {
        try {
            const certInfo = await fetchMtlsCertInfo(endpoint, timeoutMs, Logger);
