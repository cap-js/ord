...
const COMPILERS = Object.freeze({
    ...
    a2a: async function (csn, options) {
        return {
            contentType: "application/json",
            response: cds.compile(csn).to["a2a"](csn, { ...options, as: "object", ...(cds.env.ord?.compileOptions?.a2a || {}) }),
        };
    },
    oas3: function (csn, options) {
        // Check for service-level @OpenAPI.servers annotation
        const originalProtocol = csn?.definitions?.[options.service]?.["@protocol"];
        ...
        try {
            ...
            if (typeof originalProtocol !== "undefined") {
                csn.definitions[options.service]["@protocol"] = originalProtocol;
            } else {
                delete csn?.definitions?.[options.service]?.["@protocol"];
            }
        }
    },
