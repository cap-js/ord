...
const { isPrimaryDataProductService, isAgentService, agentSlug } = require("./common/utils");
...
function resolveApiResourceProtocol(srvDefinition) {
    ...
    // 2. Agent service - early return
    // TODO: IMO this is not correct: a2a is just a protocol like any-other;
    // TODO: > `@agent` is NOT LIKE @dataproduct: A2A is just any service and may be @odata at the same time.
    if (isAgentService(srvDefinition)) {
        return [
            {
                entryPoints: [`/a2a/${agentSlug(srvDefinition.name)}`],
                hasResourceDefinitions: true,
