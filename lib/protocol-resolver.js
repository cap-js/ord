...
function resolveApiResourceProtocol(srvDefinition) {
    // 1. Primary Data Product; Data Products are separate; Dataproducts may not specify other resources;
    ...
    // 2. Process all available cds.service.protocols that can be mapped to an endpoint; 
    //    > ... including such protocols injected by other plugins, that configure `path`
    ...
    // 3. Process all protocols that remain un-processed as of yet 
    const cdsProtocols = Object.keys(cds.service.protocols.for(srvDefinition));
    for (const protocol of cdsProtocols) {

        // 3.1. Handle explicit protocol - check if it's an ORD-only protocol (e.g., INA)
        ...
        // 3.2. Handle explicit protocol with no CAP endpoint (Rule A)
