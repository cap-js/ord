...
function generateIntegrationDependency(appFoundationConfig, appConfig, consumedOrdId, dataProductConfig, packageIds) {
    try {
        ...
        const appNamespace = appFoundationConfig.overrides?.commercial?.["application-namespace"];
        if (!appNamespace) {
            throw new Error(
                "commercial.application-namespace is required in app.yaml for Integration Dependency generation",
            );
        }
...
function generateIntegrationDependencies(appFoundationConfig, appConfig, packageIds) {
    if (!appFoundationConfig?.overrides?.dataProducts?.consumption) {
        Logger.log("No data products consumption configuration found in app.yaml");
        return [];
    }

    const dataProducts = appFoundationConfig.overrides.dataProducts.consumption;
