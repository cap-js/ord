...
const { isEventBrokerConfigured, getEventBrokerNamespace } = require("./eventBrokerAdapter");
...
function createEventIntegrationDependency(eventServices, appConfig, packageIds) {
    ...
    // Collect all event types from all services
    const allEventTypes = [];
    for (const service of eventServices) {
            ...
            eventResources: [
                {
                    ...
                    subset: allEventTypes.map((eventType) => ({ eventType })),
                },
            ],
        },
    ];
    ...
    return {
        ...
        title: "Consumed Events",
        shortDescription: "Integration dependency for consumed events from external systems",
        version: "1.0.0",
        releaseStatus: "active",
        visibility: RESOURCE_VISIBILITY.public,
        mandatory: false,
