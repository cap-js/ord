...
function patchGeneratedOrdResources(destinationObj, sourceObj, mergeKey = CONTENT_MERGE_KEY) {
    const destObj = _.keyBy(destinationObj, mergeKey);
    const srcObj = _.keyBy(sourceObj, mergeKey);
...
function compareAndHandleCustomORDContentWithExistingContent(ordContent, customORDContent) {
    ...
    for (const key in customORDContent) {
        ...
        if (key in ordContent) {
            // Use 'groupId' as merge key for groups, 'ordId' for all other resources
            const mergeKey = key === "groups" ? "groupId" : CONTENT_MERGE_KEY;
            clonedOrdContent[key] = patchGeneratedOrdResources(clonedOrdContent[key], customORDContent[key], mergeKey);
        } else {
