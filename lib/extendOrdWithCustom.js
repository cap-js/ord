...
function compareAndHandleCustomORDContentWithExistingContent(ordContent, customORDContent) {
    ...
    // Dkom
    const packageId = clonedOrdContent.packages.find(pkg => pkg.ordId.includes('API'))?.ordId;
    for (const entityTypes of clonedOrdContent.entityTypes) {
        if (!entityTypes.partOfPackage) {
            entityTypes.partOfPackage = packageId;
        }
    }
