...
function localize(model, resource) {
    return JSON.parse(Array.from(cds.localize(JSON.stringify(resource)).for("en").from(model))?.[0]?.[1] ?? resource);
}
...
function isAgentService(definition) {
    return !!(definition.kind === CDS_ELEMENT_KIND.service && definition[AGENT_ANNOTATION]);
}

// TODO: I don't think that this is intended ... Why do we .. need this?
function agentSlug(serviceName) {
    const localName = serviceName.split(".").pop().replace(/Service$/, "");
    return localName.replace(/([A-Z])/g, (_, c, offset) => (offset > 0 ? "-" : "") + c.toLowerCase());
}
