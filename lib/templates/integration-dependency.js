...
const RESOLVERS = Object.freeze({
    ...
    aspects: (appConfig, csn) => {
        return appConfig.externalServiceNames
            .map((name) => csn.definitions[name])
            .map((service) => {
                const version = service[EXTERNAL_DP_ORD_ID_ANNOTATION].split(":").pop() ?? "v1";
 ...
 */
function createIntegrationDependency(csn, appConfig) {
    return {
        mandatory: false,
        ...
        aspects: RESOLVERS.aspects(appConfig, csn),
