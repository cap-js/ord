...
const RESOURCE_DEFINITION_PROVIDERS = Object.freeze({
    ...
    [ORD_API_PROTOCOL.A2A]: (service, ordId, accessStrategies, entryPoints) => {
        const entryPoint = entryPoints?.[0]?.replace(/\/$/, "");
        return [
            {
                type: AGENT_CARD_RESOURCE_DEFINITION_TYPE,
                mediaType: "application/json",
                accessStrategies: accessStrategies,
                url: `${entryPoint}/.well-known/agent-card.json`,
            },
        ];
    },
...
const createAPIResourceTemplate = (srvDefinition, appConfig) => {
    ...
    protocolResults.forEach((protocolResult, index) => {
        ...
        const resourceDefinitions = !hasResourceDefinitions
            ? []
            : RESOURCE_DEFINITION_PROVIDERS[apiProtocol](srvDefinition, ordId, accessStrategies, entryPoints);
        ...
        let obj = {
            ...
            releaseStatus: apiProtocol === ORD_API_PROTOCOL.A2A ? "beta" : "active",
            apiProtocol,
            resourceDefinitions,
            entryPoints,
            extensible: {
