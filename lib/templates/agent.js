...
const RESOLVERS = Object.freeze({
    ...
    exposedApiResources: (service, appConfig) => {
        if (service["@ORD.Extensions.exposedApiResources"]) {
            return service["@ORD.Extensions.exposedApiResources"];
        }
        const name = resolveServiceName(appConfig, service);
        const version = `v${RESOLVERS.version(service).split(".")[0]}`;
        return [{ ordId: `${appConfig.ordNamespace}:apiResource:${name}:${version}` }];
    },
});
