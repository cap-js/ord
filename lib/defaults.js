...
module.exports = {
    ...
    entityTypes: [],
    get baseTemplate() {
        // Use the new authentication module instead of deprecated cds.context
        const { getAuthConfigSync } = require("./auth/authentication");
        let accessStrategies = [];

        try {
            const authConfig = getAuthConfigSync();
            accessStrategies = authConfig.accessStrategies || [];
        } catch {
            // If auth config fails to load, fall back to empty array
