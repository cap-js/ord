...
function _getPackageID(namespace, packageIds, resourceType, visibility = RESOURCE_VISIBILITY.public) {
    ...
    // When resourceType is provided, look for packages matching the resource type pattern
    if (resourceType) {
        result =
            packageIds.find((id) => {
    ...
    } else if (visibility === RESOURCE_VISIBILITY.public) {
        // When no resourceType, find package by visibility suffix
        // For public: find any package without -internal or -private suffix
        result =
            packageIds.find((id) => !id.includes("-internal") && !id.includes("-private")) ||
            packageIds.find((id) => id.includes(namespace));
    } else {
        // For internal/private: find package with matching suffix
        result =
            packageIds.find((id) => id.includes(`-${visibility}`)) || packageIds.find((id) => id.includes(namespace));
    }

    if (!result) {
        Logger.log(
            `_getPackageID: No matching package found for namespace='${namespace}', resourceType='${resourceType}', visibility='${visibility}'`,
        );
    }
...
/**
 * This is a template function to create an Integration Dependency object
 * for applications consuming events via SAP Cloud Application Event Hub.
 *
 * Integration Dependencies describe external system integrations,
 * specifically which event types the application subscribes to.
 *
 * Package selection follows the same visibility-based logic as other ORD resources:
 * the template uses _getPackageID to find the appropriate package based on visibility.
 *
 * @param {Object} options - Configuration options
 * @param {string} options.ordNamespace - The ORD namespace for this application (used for integrationDependency ordId)
 * @param {string} options.sourceNamespace - The namespace of the event source (used for eventResource ordId)
 * @param {Array<string>} options.eventTypes - Array of consumed event type strings
 * @param {Array<string>} options.packageIds - Array of available package ordIds
 * @param {string} options.packageVersion - The version for this integration dependency
 * @param {Object} [options.ordExtensions] - Optional ORD extensions for customization
 * @returns {Object} An Integration Dependency object conforming to ORD spec
 */
const createIntegrationDependencyTemplate = (options) => {
    const { ordNamespace, sourceNamespace, eventTypes, packageIds, packageVersion, ordExtensions = {} } = options;

    const visibility = ordExtensions.visibility || RESOURCE_VISIBILITY.public;

    // Use visibility-based package selection (consistent with MCP and other resources)
    const packageOrdId = _getPackageID(ordNamespace, packageIds, null, visibility);

    if (!ordNamespace || !sourceNamespace || !packageOrdId || !packageVersion) {
        Logger.warn("Integration Dependency creation skipped: missing required parameters");
        return null;
    }

    if (!eventTypes || eventTypes.length === 0) {
        Logger.log("Integration Dependency creation skipped: no event types to document");
        return null;
    }

    const integrationDependency = {
        ordId: `${ordNamespace}:integrationDependency:RawEvent:v1`,
        title: ordExtensions.title || "Customer Integration Needs",
        shortDescription: ordExtensions.shortDescription || "Integration dependency for Event Hub event subscriptions",
        partOfPackage: packageOrdId,
        version: packageVersion,
        visibility,
        releaseStatus: ordExtensions.releaseStatus || "active",
        mandatory: ordExtensions.mandatory ?? false,
        aspects: [
            {
                title: "RawEvent",
                mandatory: false,
                eventResources: [
                    {
                        ...
                        subset: eventTypes.map((eventType) => ({ eventType })),
                    },
                ],
            },
        ],
    ...
    // Allow additional ordExtensions to override defaults
    if (ordExtensions.description) {
        integrationDependency.description = ordExtensions.description;
    }
