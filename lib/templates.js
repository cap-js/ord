...
const { AUTH_TYPE_ORD_ACCESS_STRATEGY_MAP } = require("./constants");
 ...
 * @private
 */
function _getResourceDefinition(resourceType, mediaType, ordId, serviceName, fileExtension) {
    return {
        type: resourceType,
        mediaType: `application/${mediaType}`,
        url: `/ord/v1/${ordId}/${serviceName}.${fileExtension}`,
        accessStrategies: cds.context.authConfig.types.map((type) => ({
...
const createAPIResourceTemplate = (serviceName, serviceDefinition, appConfig, packageIds) => {
    ...
    paths.forEach((generatedPath) => {
        const resourceDefinition = _getResourceDefinition("openapi-v3", "json", ordId, serviceName, "oas3.json");
        ...
        if (generatedPath.kind !== "rest") {
            ...
            resourceDefinitions.push(_getResourceDefinition("edmx", "xml", ordId, serviceName, "edmx"));
        }
...
const createEventResourceTemplate = (serviceName, serviceDefinition, appConfig, packageIds) => {
    ...
    return [{
        ...
        partOfGroups: [_getGroupID(serviceName, defaults.groupTypeId, appConfig)],
        visibility: RESOURCE_VISIBILITY.public,
        resourceDefinitions: [_getResourceDefinition("asyncapi-v2", "json", ordId, serviceName, "asyncapi2.json")],
