...
const fCreateAPIResourceTemplate = (srv) => {
    ...
    return {
        ordId: `${global.packageNameReg}:apiResource:${global.namespace === undefined ? `${srv}`  : `${global.namespace}.${srv}`}:v1`,
...
const fCreateIntegrationDependencyTemplate = (packageNameReg,name) => {
    ...
    const ordExtent = fReadORDExtensions(cds.services[name].definition);
    const events = Object.values(cds.services[name].events).map(ele => ({ eventType: ele['@topic'] }))
    return {
        ordId: `${packageNameReg}:integrationDependency:${name}:v1`,
        title: ordExtent.title ?? "Integration Dependency sample title",
        partOfPackage: "sap.xref:package:SomePackage:v1",
        version: ordExtent.version ?? "1.0.0",
        visibility: ordExtent.visibility ?? "public",
        releaseStatus: ordExtent.releaseStatus ?? "active",
        mandatory: ordExtent.mandatory ?? true,
        aspects: [
            {
                title: 'test title',
                mandatory: true,
