...
const { isMCPPluginAvailable, isMCPPluginInPackageJson } = require("./mcpAdapter");
...
/**
 * This is a template function to create MCP API Resource object.
 *
 * The generated MCP API resource can be customized via the generic overwrite mechanism
 * using custom.ord.json. Properties like visibility, title, version, entryPoints, etc.
 * can be overridden by matching the ordId pattern: "{namespace}:apiResource:mcp-server:v1"
 *
...
const createMCPAPIResourceTemplate = (appConfig, packageIds, accessStrategies) => {
    const ordId = `${appConfig.ordNamespace}:apiResource:mcp-server:v1`;

    // Get ORD metadata from MCP plugin if available
    let ordMetadata = null;
    // Use package.json detection for build-time scenarios, runtime detection for runtime scenarios
    const shouldAddMCP = isMCPPluginInPackageJson() && isMCPPluginAvailable();
    if (shouldAddMCP) {
        try {
    ...
    const title = ordMetadata?.title || `MCP Server for ${appConfig.appName}`;
    const shortDescription =
        ordMetadata?.shortDescription || `This is the MCP server to interact with the ${appConfig.appName}`;
    const description = ordMetadata?.description || `This is the MCP server to interact with the ${appConfig.appName}`;
    const version = ordMetadata?.version || "1.0.0";
    const visibility = ordMetadata?.visibility || RESOURCE_VISIBILITY.public;

    // Handle entryPoints: convert string to array, with fallback
    const entryPoints = ordMetadata?.entryPoints ? ordMetadata.entryPoints : ["/rest/mcp/streaming"];
