...
const createEntityTypeMappingsItemTemplate = (entity) => {
    ...
    } else if (entity[ORD_ODM_ENTITY_NAME_ANNOTATION]) {
        return {
...
const createDataProductAPIResourceTemplate = (dataProduct) => {
    return {
        ordId: "sap.epd:apiResource:Product:v1", // TODO: `${dataProduct.ordNamespace}:apiResource:${dataProduct.name}:v1` is not unique in both cases ??
        title:
            dataProduct["@title"] ??
            dataProduct["@Common.Label"] ??
            dataProduct.name,
        shortDescription: SHORT_DESCRIPTION_PREFIX + dataProduct.name,
        description: dataProduct["@Core.Description"] ?? DESCRIPTION_PREFIX + dataProduct.name,
        version: "1.0.0",
        lastUpdate: dataProduct.lastUpdate,
        ...
        // conditionally setting the entityTypeMappings field based on the presence of entityTypeTargets in appConfig
        ...(dataProduct.entityTypeTargets?.length > 0 &&
        {
            entityTypeMappings: [
                {
                    entityTypeTargets: dataProduct.entityTypeTargets.map(et => {
                        return (
                            {
                                "ordId": et.ordId
                            }
