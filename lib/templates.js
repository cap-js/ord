...
const fCreateAPIResourceTemplate = (srv, srvDefinition) => {
  ...
  if (checkEntityFunctionAction(srvDefinition).length > 0) {
    return {
      ...
      title: ordExtensions.title ?? srvDefinition['@title'] ?? `The service is for ${srv}`,
      shortDescription: ordExtensions.shortDescription ?? 
                        srvDefinition['@shortDescription'] ?? 
                        `Here we have the shortDescription for ${srv}`,
      description: ordExtensions.description ?? 
                   `Here we have the description for ${srv}`,
      version: ordExtensions.version ?? srvDefinition["@version"] ?? "1.0.0",
      visibility: ordExtensions.visibility ?? srvDefinition["@visibility"] ?? "public",
      partOfPackage: `${global.namespace}:service:${global.capNamespace}.${srv}:v1`,
      partOfGroups: _getGroupID(ordExtensions, srv),
      releaseStatus: ordExtensions.active ?? srvDefinition["@releaseStatus"] ?? "active",
      partOfConsumptionBundles: [
        {
          ordId: `${global.namespace}:consumptionBundle:noAuth:v1`,
        },
      ],
      apiProtocol: ordExtensions.apiProtocol ?? srvDefinition['@apiProtocol'] ?? "odata-v4",
      resourceDefinitions: [
        {
          type: "openapi-v3",
      ...
      entryPoints: paths,
      extensible: { supported: ordExtensions["extensible.supported"] ?? srvDefinition['@extensible.supported'] ?? "no" },
...
const fCreateEventResourceTemplate = (srv, srvDefinition) => {
  ...
  return {
    ...
    title:  ordExtensions.title ?? srvDefinition['@title'] ?? `ODM ${global.appName} Events`,
    shortDescription: ordExtensions.shortDescription ?? srvDefinition['@shortDescription'] ?? "Example ODM Event",
    description:  ordExtensions.description ?? 
                  srvDefinition['@description'] ?? 
                  `This is an example event catalog that contains only a partial ODM ${global.appName} V1 event`,
    version:  ordExtensions.version ?? srvDefinition["@version"] ?? "1.0.0",
    releaseStatus:  ordExtensions.releaseStatus ?? srvDefinition["@releaseStatus"] ?? "beta",
