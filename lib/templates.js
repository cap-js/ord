...
const createAPIResourceTemplate = (serviceName, serviceDefinition, appConfig, packageIds, accessStrategies) => {
    ...
    paths.forEach((generatedPath) => {
        ...
        let obj = {
            ...
            entityTypeMappings: _getEntityTypeMappings(serviceDefinition),
            ...ordExtensions,
        };
...
function _getEntityTypeMappings(serviceDefinition) {
    const entities = Object.values(serviceDefinition.entities).flatMap(
        entity => _assertionsFlatList(entity)
    ).map(createEntityTypeMappingsItemTemplate)
    const entityTypeTargets = _.uniqBy(entities, CONTENT_MERGE_KEY).filter(entity => entity !== undefined).map(({ordId}) => ({
        ordId
    }))
...
function _assertionsFlatList(currentEntity, processedEntities = []) {
    if(!currentEntity.associations) {
        return [currentEntity]
    }
    const entityAssociationTargets = Object.values(currentEntity.associations).map(association => ({target: association.target, entity: association._target}))
    ...
    return [currentEntity, ...assertionsTodo.flatMap(entity => _assertionsFlatList(entity, processedEntities))]
}
