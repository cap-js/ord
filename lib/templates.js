...
const createEntityTypeTemplate = (appConfig, packageIds, entity) => {
    ...
    // Check whether the EntityType is referenced by a restrictive API
    const referencedByRestrictedAPI = (Array.isArray(appConfig.apiResources) ? appConfig.apiResources : []).some(api =>
        api.entityTypeMappings?.some(mapping =>
            mapping.entityTypeTargets.some(target => target.ordId === entity.ordId)
        ) && [RESOURCE_VISIBILITY.private, RESOURCE_VISIBILITY.internal].includes(api.visibility)
    );

   // Check whether the EntityType is referenced by a restrictive event resource
    const referencedByRestrictedEvent = (Array.isArray(appConfig.eventResources) ? appConfig.eventResources : []).some(event =>
        event.entityTypeMappings?.some(mapping =>
            mapping.entityTypeTargets.some(target => target.ordId === entity.ordId)
        ) && [RESOURCE_VISIBILITY.private, RESOURCE_VISIBILITY.internal].includes(event.visibility)
    );

    // Check whether the EntityType is referenced by a restrictive DataProduct
    const referencedByRestrictedDataProduct = (Array.isArray(appConfig.dataProducts) ? appConfig.dataProducts : []).some(dp =>
        dp.entityTypes?.some(target => target === entity.ordId)
        && [RESOURCE_VISIBILITY.private, RESOURCE_VISIBILITY.internal].includes(dp.visibility)
    );

    // Adopt the most restrictive visibility
    let finalVisibility = visibility;
    if (referencedByRestrictedAPI || referencedByRestrictedEvent || referencedByRestrictedDataProduct) {
        if (referencedByRestrictedAPI && appConfig.apiResources.some(api => api.visibility === RESOURCE_VISIBILITY.private) ||
            referencedByRestrictedEvent && appConfig.eventResources.some(event => event.visibility === RESOURCE_VISIBILITY.private) ||
            referencedByRestrictedDataProduct && appConfig.dataProducts.some(dp => dp.visibility === RESOURCE_VISIBILITY.private)) {
            finalVisibility = RESOURCE_VISIBILITY.private;
        } else {
            finalVisibility = RESOURCE_VISIBILITY.internal;
        }
    }

    if (finalVisibility === RESOURCE_VISIBILITY.private) {
        return null;
    }

    const packageId = packageIds.find(id =>
        finalVisibility === RESOURCE_VISIBILITY.public
            ? id.includes(`entityType`) && (!id.includes("-internal") && !id.includes("-private"))
            : id.includes(`entityType-${finalVisibility}`)
    );
    ...
    return {
        ...
        partOfPackage: packageId || _getPackageID(appConfig.ordNamespace, packageIds, ORD_RESOURCE_TYPE.entityType),
...
const createAPIResourceTemplate = (serviceName, serviceDefinition, appConfig, packageIds, accessStrategies) => {
    const ordExtensions = readORDExtensions(serviceDefinition);
    const visibility = ordExtensions.visibility || RESOURCE_VISIBILITY.public;
    const packageId = Array.isArray(packageIds)
    ? packageIds.find(id =>
        visibility === RESOURCE_VISIBILITY.public
            ? id.includes("api") && !id.includes("-internal") && !id.includes("-private")
            : id.includes(`api-${visibility}`)
    )
    : null;
    ...
    paths.forEach((generatedPath) => {
        ...
        // Ensure API resource respects referenced EntityType visibility
        const referencedEntityTypes = Array.isArray(appConfig.entityTypeMappings)
            ? appConfig.entityTypeMappings.flatMap(mapping => mapping.entityTypeTargets)
                .filter(target => target.ordId.startsWith(appConfig.ordNamespace))
            : [];

        const hasRestrictedEntityType = Array.isArray(appConfig.entityTypeMappings)
            ? appConfig.entityTypeMappings.some(mapping =>
                mapping.entityTypeTargets.some(target =>
                    referencedEntityTypes.some(et => et.ordId === target.ordId) &&
                    [RESOURCE_VISIBILITY.private, RESOURCE_VISIBILITY.internal].includes(target.visibility)
                )
            )
            : false;
            ...
            if (hasRestrictedEntityType) {
                if (referencedEntityTypes.some(et => et.visibility === RESOURCE_VISIBILITY.private)) {
                    finalVisibility = RESOURCE_VISIBILITY.private;
                } else {
                    finalVisibility = RESOURCE_VISIBILITY.internal;
                }
            ...
            // Make sure that the EntityType package is also “internal” if an associated API is internal
            const entityTypePackageId = referencedEntityTypes.length > 0
            ? packageIds.find(id =>
                finalVisibility === RESOURCE_VISIBILITY.public
                    ? id.includes("entityType") && !id.includes("-internal") && !id.includes("-private")
                    : id.includes(`entityType-internal`) // If the API is internal, entityType is also internal
            )
            : null;
        ...
        let obj = {
            ...
            lastUpdate: appConfig.lastUpdate,
            visibility: finalVisibility,
            partOfPackage: packageId || _getPackageID(appConfig.ordNamespace, packageIds, ORD_RESOURCE_TYPE.api),
            ...
            integrationDependency: Array.isArray(packageIds)
            ? packageIds.find(id =>
                id.includes("integrationDependency") &&
                (finalVisibility === RESOURCE_VISIBILITY.public ? !id.includes("-internal") : id.includes("-internal"))
            ) || null
            : null,
            entityTypeMappings: referencedEntityTypes.length > 0
                ? [{
                    entityTypeTargets: _.uniqBy(referencedEntityTypes.map(et => ({ "ordId": et.ordId })), 'ordId'),
                    partOfPackage: entityTypePackageId // Jetzt wird sichergestellt, dass das Paket richtig zugewiesen ist
                }]
                : undefined,
            ...ordExtensions,
        };
...
const createEventResourceTemplate = (serviceName, serviceDefinition, appConfig, packageIds, accessStrategies) => {
    ...
    ? packageIds.find(id =>
        visibility === RESOURCE_VISIBILITY.public
            ? id.includes("event") && !id.includes("-internal") && !id.includes("-private")
            : id.includes(`event-${visibility}`)
    )
    : null;

    const ordId = `${appConfig.ordNamespace}:eventResource:${serviceName}:v1`;

      // Ensure Event resource respects referenced EntityType visibility
    const referencedEntityTypes = Array.isArray(appConfig.entityTypeMappings)
    ? appConfig.entityTypeMappings.flatMap(mapping => mapping.entityTypeTargets)
        .filter(target => target.ordId.startsWith(appConfig.ordNamespace))
    : [];
    ...
    ? packageIds.find(id =>
        finalVisibility === RESOURCE_VISIBILITY.public
            ? id.includes("entityType") && !id.includes("-internal") && !id.includes("-private")
            : id.includes("entityType-internal") // Falls das Event internal ist, wird auch das entityType-Paket internal
    )
    : null;
    ...
    return [{
        ...
        lastUpdate: appConfig.lastUpdate,
        releaseStatus: "active",
        partOfPackage: packageId || _getPackageID(appConfig.ordNamespace, packageIds, ORD_RESOURCE_TYPE.event),
        partOfGroups: [_getGroupID(serviceName, defaults.groupTypeId, appConfig)],
        visibility: finalVisibility,
        ...
        entityTypeMappings: referencedEntityTypes.length > 0
            ? [{
                entityTypeTargets: _.uniqBy(referencedEntityTypes.map(et => ({ "ordId": et.ordId })), 'ordId'),
                partOfPackage: entityTypePackageId // Korrekte Paketzuweisung für EntityTypes
            }]
            : undefined,
        ...ordExtensions
    }]
...
/**
 * This is a template function to create Data Product object for Data Products Array.
 * It ensures correct visibility and package assignment.
 *
 * @param {string} dataProductName The name of the Data Product.
 * @param {object} dataProductDefinition The definition of the Data Product.
 * @param {object} appConfig The application configuration.
 * @param {Array} packageIds The list of available package IDs.
 * @returns {Array} An array of objects for the Data Products.
 */
const createDataProductTemplate = (dataProductName, dataProductDefinition, appConfig, packageIds) => {
    const ordExtensions = readORDExtensions(dataProductDefinition);
    const visibility = ordExtensions.visibility || RESOURCE_VISIBILITY.public;
    ...
    const packageId = Array.isArray(packageIds)
        ? packageIds.find(id =>
            finalVisibility === RESOURCE_VISIBILITY.public
                ? id.includes("dataProduct") && !id.includes("-internal") && !id.includes("-private")
                : id.includes(`dataProduct-${finalVisibility}`)
        )
        : null;

    const ordId = `${appConfig.ordNamespace}:dataProduct:${dataProductName}:v1`;

    const entityTypePackageId = referencedEntityTypes.length > 0
    ? packageIds.find(id =>
        finalVisibility === RESOURCE_VISIBILITY.public
            ? id.includes("entityType") && !id.includes("-internal") && !id.includes("-private")
            : id.includes("entityType-internal") // Falls das Data Product internal ist, wird auch das entityType-Paket internal
    )
    : null;

    return [{
        ordId,
        title: dataProductDefinition["@title"] ?? dataProductName,
        shortDescription: dataProductDefinition["@shortDescription"] ?? `${dataProductName} Data Product`,
        description: dataProductDefinition["@description"] ?? `Data Product for ${dataProductName}.`,
        version: "1.0.0",
        lastUpdate: appConfig.lastUpdate,
        visibility: finalVisibility,
        partOfPackage: packageId || _getPackageID(appConfig.ordNamespace, packageIds, ORD_RESOURCE_TYPE.dataProduct),
        releaseStatus: "active",
        type: "primary",
        category: "business-object",
        outputPorts: ordExtensions.outputPorts || [],
        entityTypeMappings: referencedEntityTypes.length > 0
            ? [{
                entityTypeTargets: _.uniqBy(referencedEntityTypes.map(et => ({ "ordId": et.ordId })), 'ordId'),
                partOfPackage: entityTypePackageId // Korrekte Paketzuweisung für EntityTypes
