...
const fCreateAPIResourceTemplate = (srv, srvDefinition) => {
  ...
  if (checkEntityFunctionAction(srvDefinition).length > 0) {
    return {
      ordId: `${global.namespace}:apiResource:${global.capNamespace}.${srv}:v1`,
      title: ordExtent.title ?? `The service is for ${srv}`,
      shortDescription:
        ordExtent.shortDescription ??
        `Here we have the shortDescription for ${srv}`,
      description:
        ordExtent.description ?? `Here we have the description for ${srv}`,
