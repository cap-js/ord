...
function _getGroupID(serviceDefinition, groupTypeId = defaults.groupTypeId, appConfig) {
    return `${groupTypeId}:${appConfig.ordNamespace}:${_getCleanServiceName(serviceDefinition.name, appConfig)}`;
}
...
const createAPIResourceTemplate = (serviceName, serviceDefinition, appConfig, packageIds, accessStrategies) => {
    ...
    const baseOrdId = ordExtensions.ordId || `${appConfig.ordNamespace}:apiResource:${cleanServiceName}:${version}`;

    const perProtocolExtensions = ordExtensions.apiResources || {};
    const { apiResources: _, ...ordExtensionsBase } = ordExtensions;

    if (protocolResults.length > 1 && Object.keys(perProtocolExtensions).length === 0) {
        Logger.warn(
            `Service '${serviceName}' exposes multiple protocols. ` +
                `Consider providing per-protocol ordId overwrites via @ORD.Extensions.apiResources to ensure stable identifiers.`,
        );
    }

    protocolResults.forEach((protocolResult, index) => {
        const { apiProtocol, entryPoints, hasResourceDefinitions } = protocolResult;
        const protocolOverrides = perProtocolExtensions[apiProtocol] || {};
        const ordId = protocolOverrides.ordId || (index === 0 ? baseOrdId : `${baseOrdId}-${apiProtocol}`);
...
const createEventResourceTemplate = (serviceName, serviceDefinition, appConfig, packageIds, accessStrategies) => {
    ...
    const ordId = `${appConfig.ordNamespace}:eventResource:${_getCleanServiceName(serviceDefinition.name, appConfig)}:v${version.split(".")[0]}`;
    const exposedEntityTypes = _getExposedEntityTypes(serviceDefinition);
