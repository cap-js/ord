...
const createAPIResourceTemplate = (serviceName, serviceDefinition, appConfig, packageIds, accessStrategies) => {
    ...
    protocolResults.forEach((protocolResult, index) => {
        ...
        const resourceId = index === 0 ? cleanServiceName : `${cleanServiceName}-${apiProtocol}`;
        const ordId = ordExtensions.ordId || `${appConfig.ordNamespace}:apiResource:${resourceId}:${version}`;
