...
const createEntityTypeTemplate = (appConfig, packageIds, entity) => {
    ...
    // Determine if this EntityType is referenced by a private DataProduct or API/Event Resource
    const referencedByRestrictedAPI = (Array.isArray(appConfig.apiResources) ? appConfig.apiResources : []).some(api =>
        api.entityTypeMappings?.some(mapping =>
            mapping.entityTypeTargets.some(target => target.ordId === entity.ordId &&
                (api.visibility === RESOURCE_VISIBILITY.private || api.visibility === RESOURCE_VISIBILITY.internal))
        )
    );
    ...
    const referencedByRestrictedEvent = (Array.isArray(appConfig.eventResources) ? appConfig.eventResources : []).some(event =>
        event.entityTypeMappings?.some(mapping =>
            mapping.entityTypeTargets.some(target => target.ordId === entity.ordId &&
                (event.visibility === RESOURCE_VISIBILITY.private || event.visibility === RESOURCE_VISIBILITY.internal))
        )
    );
    ...
    const referencedByRestrictedDataProduct = (Array.isArray(appConfig.dataProducts) ? appConfig.dataProducts : []).some(dp =>
        dp.entityTypes?.some(target => target === entity.ordId &&
            (dp.visibility === RESOURCE_VISIBILITY.private || dp.visibility === RESOURCE_VISIBILITY.internal))
    );

    const finalVisibility = referencedByRestrictedAPI || referencedByRestrictedEvent || referencedByRestrictedDataProduct
        ? (visibility === RESOURCE_VISIBILITY.private ? RESOURCE_VISIBILITY.private : RESOURCE_VISIBILITY.internal)
        : visibility;
...
const createAPIResourceTemplate = (serviceName, serviceDefinition, appConfig, packageIds, accessStrategies) => {
    ...
    paths.forEach((generatedPath) => {
        ...
        const referencedEntityTypes = Array.isArray(appConfig.entityTypes)
            ? appConfig.entityTypes.filter(et =>
                et.entityTypeMappings?.some(mapping =>
                    mapping.entityTypeTargets.some(target => target.ordId === ordId)
                )
            )
            : [];

        const hasRestrictedEntityType = referencedEntityTypes.some(et =>
            et.visibility === RESOURCE_VISIBILITY.private || et.visibility === RESOURCE_VISIBILITY.internal
        );
        const finalVisibility = hasRestrictedEntityType
        ? (visibility === RESOURCE_VISIBILITY.private ? RESOURCE_VISIBILITY.private : RESOURCE_VISIBILITY.internal)
        : visibility;
...
const createDataProductTemplate = (dataProductName, dataProductDefinition, appConfig, packageIds) => {
    ...
    // Check if any referenced entityType is private → force DataProduct to be private
    const referencedEntityTypes = ordExtensions.entityTypes || [];
    const hasRestrictedEntityType = Array.isArray(appConfig.entityTypes)
        ? referencedEntityTypes.some(entityType =>
            [RESOURCE_VISIBILITY.private, RESOURCE_VISIBILITY.internal].includes(
                appConfig.entityTypes.find(et => et.ordId === entityType.ordId)?.visibility
            )
        )
        : false;
    ...
    return [{
        ...
        entityTypes: referencedEntityTypes,
        integrationDependency: Array.isArray(packageIds)
            ? packageIds.find(id =>
                id.includes("integrationDependency") &&
                (finalVisibility === RESOURCE_VISIBILITY.public ? !id.includes("-internal") : id.includes("-internal"))
