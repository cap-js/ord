...
const _generatePaths = (srv, srvDefinition) => {
    ...
    Logger.info(`[ORD Debug] _generatePaths called for service: ${srv}`);
    Logger.info(`[ORD Debug] Service @protocol annotation: ${JSON.stringify(srvDefinition["@protocol"])}`);
    Logger.info(`[ORD Debug] Service @odata annotation: ${JSON.stringify(srvDefinition["@odata"])}`);
    Logger.info(`[ORD Debug] Service @rest annotation: ${JSON.stringify(srvDefinition["@rest"])}`);

    const paths = protocols.endpoints4(srvObj);

    // Debug logging for endpoints4 result
    Logger.info(`[ORD Debug] endpoints4 returned ${paths.length} paths for ${srv}: ${JSON.stringify(paths)}`);
    ...
    if (paths.length === 0) {
        if (isPrimaryDataProductService(srvDefinition)) {
            ...
            Logger.info(`[ORD Debug] Added default REST path for data product service: ${srv}`);
        } else {
            srvDefinition["@odata"] = true;
            paths.push({ kind: "odata", path: protocols.path4(srvDefinition) });
            Logger.info(`[ORD Debug] Added default OData path for service: ${srv}`);
        }
    }

    Logger.info(`[ORD Debug] Final paths for ${srv}: ${JSON.stringify(paths)}`);
    return paths;
};
