...
/**
 * Creates an integration dependency template for consumed external services or events.
 *
 * @param {string} serviceName - The name of the external service being consumed
 * @param {Object} serviceDefinition - The service definition object
 * @param {Object} appConfig - Application configuration
 * @param {Array} packageIds - Array of package IDs
 * @param {Array} accessStrategies - Access strategies
 * @returns {Array} Array containing the integration dependency object
 */
const createIntegrationDependencyTemplate = (serviceName, serviceDefinition, appConfig, packageIds, accessStrategies) => {
    const ordExtensions = readORDExtensions(serviceDefinition);
    const visibility = _handleVisibility(ordExtensions, serviceDefinition, appConfig.env?.defaultVisibility);
    const packageId = _getPackageID(appConfig.ordNamespace, packageIds, ORD_RESOURCE_TYPE.integrationDependency, visibility);
    const ordId = `${appConfig.ordNamespace}:integrationDependency:${_getGroupNameWithNestedNamespace(serviceDefinition, appConfig)}:v1`;

    let obj = {
        ordId,
        title: serviceDefinition["@title"] ??
               serviceDefinition["@Common.Label"] ??
               `${serviceName} Integration Dependency`,
        shortDescription: `Integration dependency for ${serviceName}`,
        description: serviceDefinition["@description"] ??
                    serviceDefinition["@Core.Description"] ??
                    `Integration dependency for consuming external service ${serviceName}`,
        version: "1.0.0",
        lastUpdate: appConfig.lastUpdate,
        releaseStatus: "active",
        partOfPackage: packageId,
        ...
        visibility,
        aspects: [{
            title: "External Service Consumption",
            description: `Consumes events and/or APIs from external service ${serviceName}`,
            mandatory: true,
            supportMultipleProviders: false,
            apiResources: [], // Can be populated with consumed APIs if needed
            eventResources: [] // Can be populated with consumed events if needed
        }],
        extensible: { supported: "no" },
        ...ordExtensions,
    };
