...
/**
 * Reads the path and protocol annotations from a given service definition object and returns an array of entryPoint paths.
 * 
 * @param {Object} srv The service definition object.
 * @returns {Array} An array containing paths.
 */

const fGeneratePaths = (srvName, srv) => {
    const obj = { name: srvName, definition: srv};
    const paths = [];
    const protocols = cds.service.protocols;
    
    const kindPaths = protocols.endpoints4(obj);
 ...
 */
const fCreateAPIResourceTemplate = (srv, csn) => {

    const serviceDefinition = csn.definitions[srv];
    const ordExtent =fReadORDExtensions(serviceDefinition);
    const paths = fGeneratePaths(srv, serviceDefinition);
    return {
 ...
 */
const fCreateEventResourceTemplate = (srv, csn) => {
    const ordExtent = fReadORDExtensions(csn.definitions[srv]);
    return {
        ...
        resourceDefinitions: [
            {
                type: "asyncapi-v2",
                mediaType: "application/json",
                url: `/.well-known/open-resource-discovery/v1/api-metadata/${srv}`, //${csn.definitions[srv]._service.name}.asyncapi2.json`, //
                accessStrategies: [
                    {
                        type: "open",
                    },
