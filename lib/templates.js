 ...
 * 
 * @param {string} fullyQualifiedName The fully qualified name of the service oe event.
 * @param {string} groupTypeId The group type id.
 * @returns {string} a group id.
 */
function _getGroupID(ordExtensions, srv) {
    return `${ordExtensions.groupTypeId ?? defaults.groupTypeId}:${fReplaceSpecialCharacters(global.namespace)}:${global.capNamespace}.${srv}`;
}
...
const fCreateGroupsTemplateForService = (srv, srvDefinition) => {
    ...
    if (checkEntityFunctionAction(srvDefinition).length > 0) {
        return {
            groupId: _getGroupID(fullyQualifiedServiceName, defaults.groupTypeId),
...
const fCreateGroupsTemplateForEvent = (event, eventDefinition) => {
    ...
    return {
        groupId: _getGroupID(fullyQualifiedEventName, defaults.groupTypeId),
        groupTypeId: `${defaults.groupTypeId}`,
