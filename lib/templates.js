...
const { isMCPPluginAvailable } = require("./mcpAdapter");
...
const createMCPAPIResourceTemplate = (appConfig, packageIds, accessStrategies) => {
    ...
    const shortDescription = ordMetadata?.shortDescription || `This is the MCP server to interact with the ${appConfig.appName}`;
    ...
    const entryPoints = ordMetadata?.entryPoints 
        ? [ordMetadata.entryPoints] 
        : ["/rest/mcp/streaming"];
