 ...
 * @returns {Object} A group object.
 */
const fCreateGroupsTemplateForEvent = (event, eventDefinition) => {
    ...
    let groupId = _getGroupID(fullyQualifiedEventName, defaults.groupTypeId, false);
    if (groupIds.has(groupId)) {
        return null;
    } else {
        groupIds.add(groupId);
  ...
  if (checkEntityFunctionAction(srvDefinition).length > 0) {
    paths.forEach((generatedPath) => {
      ...
      let obj = {
        ...
        partOfPackage: `${fReplaceSpecialCharacters(global.namespace)}:package:${global.capNamespace}.${srv}:v1`,
        partOfGroups: [_getGroupID(srvName, defaults.groupTypeId)],
...
const fCreateEventResourceTemplate = (srv, srvDefinition) => {
  ...
  return {
    ...
    partOfPackage: `${fReplaceSpecialCharacters(global.namespace)}:package:${global.appName.replace(/[^a-zA-Z0-9]/g, '')}:v1`,
    partOfGroups: [_getGroupID(srvName, defaults.groupTypeId, false)],
    visibility: ordExtensions.visibility ?? "public",
    resourceDefinitions: [
      {
        type: "asyncapi-v2",
...
const fCreateIntegrationDependencyTemplate = (name) => {
  const srvDefinition = cds.services[name].definition;
  const ordExtensions = fReadORDExtensions(srvDefinition);

  const events = Object.values(srvDefinition.events).filter(ele => ele['@topic']).map(ele => ({ eventType: ele['@topic'] }));
  return {
      ordId: `${global.namespace}:integrationDependency:${name}:v1`,
      title: ordExtensions.title ?? srvDefinition['@title'] ?? "Integration Dependency sample title",
      ...
      aspects: [
          {
              ...
              apiResources: [
                  {
                      ordId:`${global.namespace}:apiResource:${cds.services[name].name}:v1`
                  }
              ],
              eventResources: [
                  {
                      ordId: `${global.namespace}:eventResource:${cds.services[name].name}:v1`,
                      subset: events
                  }
              ]
          }
