...
const createIntegrationDependencyTemplate = (serviceName, serviceDefinition, appConfig, packageIds) => {

    const ordExtensions = readORDExtensions(serviceDefinition);

    const event = Object.keys(serviceDefinition.model.definitions).filter(srv => {
        return srv.startsWith(serviceName) && appConfig.events.includes(srv)
    }).map((eventName) => {
        return {
            eventType: serviceDefinition.model.definitions[eventName]["@topic"]
        }
    });

    return {
        ordId: `${appConfig.ordNamespace}:integrationDependency:${serviceName}:v1`,
        title: ordExtensions.title ?? serviceDefinition['@title'] ?? "Integration Dependency sample titile",
        ...
        aspects: [
            {
                eventResources: [
                    {
                        ordId: `${appConfig.ordNamespace}:apiResource:${serviceDefinition.name}:v1`,
                        subset: event
                    }
                ]
            }
