...
function _getGroupID(appConfig, definition) {
    return `${defaults.groupTypeId}:${appConfig.ordNamespace}:${_getCleanServiceName(definition.name, appConfig)}`;
}
...
const createGroupsTemplateForService = (serviceName, serviceDefinition, appConfig) => {
    ...
    return {
        groupId: _getGroupID(appConfig, serviceDefinition),
...
const createAPIResourceTemplate = (serviceName, serviceDefinition, appConfig, packageIds, accessStrategies) => {
    ...
    protocolResults.forEach((protocolResult) => {
        ...
        let obj = {
            ...
            visibility,
            partOfPackage: packageId,
            partOfGroups: [_getGroupID(appConfig, serviceDefinition)],
            releaseStatus: "active",
            apiProtocol,
            resourceDefinitions,
            entryPoints,
