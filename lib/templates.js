...
const createEntityTypeTemplate = (appConfig, packageIds, entity) => {
    ...
    const packageId = packageIds.find(id =>
        visibility === RESOURCE_VISIBILITY.public
            ...
            : id.includes(`entityType-${visibility}`)
    );
        ...
        let obj = {
            ...
            integrationDependency: Array.isArray(packageIds)
                ? packageIds.find(id =>
                    ...
                    (visibility === RESOURCE_VISIBILITY.public ? !id.includes("-internal") : id.includes("-internal"))
                ) || null
                : null,
            entityTypeMappings: Array.isArray(appConfig.entityTypeMappings)
                ? [{
                    entityTypeTargets: _.uniqBy(
                        appConfig.entityTypeMappings.flatMap(m => m.entityTypeTargets).map(et => ({ ordId: et.ordId })), 'ordId'
                    ),
                }]
                : [],
            ...ordExtensions,
    ...
    const packageId = Array.isArray(packageIds)
        ? packageIds.find(id =>
            visibility === RESOURCE_VISIBILITY.public
                ...
                : id.includes(`dataProduct-${visibility}`)
        )
        : null;
