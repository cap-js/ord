...
const fCreateAPIResourceTemplate = (srv, srvDefinition) => {
  ...
  if (checkEntityFunctionAction(srvDefinition).length > 0) {
    return {
      ...
      title: ordExtensions.title ?? srvDefinition['@title'] ?? srvDefinition['@Common.Label'] ?? `The service is for ${srv}`,
      shortDescription: ordExtensions.shortDescription ?? `Here we have the shortDescription for ${srv}`,
      description: ordExtensions.description ?? srvDefinition['@Core.Description'] ?? `Here we have the description for ${srv}`,
