...
const createAPIResourceTemplate = (serviceName, serviceDefinition, appConfig, packageIds, accessStrategies) => {
    ...
    const baseResourceId = cleanServiceName;
    const baseOrdId = ordExtensions.ordId;

    if (protocolResults.length > 1) {
        Logger.warn(
            `Service '${serviceName}' exposes multiple protocols. ` +
                `Consider providing explicit ordId via @ORD.Extensions.ordId to ensure stable identifiers.`,
        );
    }

    const { ordId: _, ...ordExtensionsWithoutOrdId } = ordExtensions;

    protocolResults.forEach((protocolResult, index) => {
        const { apiProtocol, entryPoints, hasResourceDefinitions } = protocolResult;
        const resourceId = index === 0 ? baseResourceId : `${baseResourceId}-${apiProtocol}`;
        const ordId =
            index === 0 && baseOrdId ? baseOrdId : `${appConfig.ordNamespace}:apiResource:${resourceId}:${version}`;
