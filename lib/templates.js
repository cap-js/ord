...
const createIntegrationDependencyTemplate = (serviceName, serviceDefinition, appConfig, packageIds) => {
    ...
    const events = Object.keys(serviceDefinition.model.definitions)
        .filter(event => {
            const definition = serviceDefinition.model.definitions[event];
            return definition["@topic"] && definition["@topic"].startsWith(serviceName) && !event.startsWith(serviceName);
        })
