 ...
 */
const createEventResourceTemplate = (definition, appConfig, packageIds, accessStrategies) => {
    ...
    const ordId = `${appConfig.ordNamespace}:eventResource:${_getCleanServiceName(definition.name, appConfig)}:v${version.split(".")[0]}`;
    ...
    let obj = {
        ...
        title:
            definition["@title"] ??
            definition["@Common.Label"] ??
            definition["@EndUserText.label"] ??
            `ODM ${appConfig.appName.replace(/[^a-zA-Z0-9]/g, "")} Events`,
        shortDescription: `${definition.name} event resource`,
        description:
        ...
        visibility,
        resourceDefinitions: [
            _getResourceDefinition("asyncapi-v2", "json", ordId, definition.name, "asyncapi2.json", accessStrategies),
        ],
        extensible: { supported: "no" },
        ...(exposedEntityTypes ? { exposedEntityTypes } : []),
        ...ordExtensions,
