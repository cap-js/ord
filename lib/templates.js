...
function _getGroupID(fullyQualifiedName, groupTypeId = defaults.groupTypeId, isService = true) {
    if (isService) {
        return `${groupTypeId}:${global.namespace}:${fullyQualifiedName}`;
    } else {
        return `${groupTypeId}:${global.namespace}:` +
...
const fCreateEventResourceTemplate = (srv, srvDefinition) => {
  ...
  return {
    ...
    title:  ordExtensions.title ?? srvDefinition['@title'] ?? srvDefinition['@Common.Label'] ?? `ODM ${global.appName} Events`,
