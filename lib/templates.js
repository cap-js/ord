...
const createEntityTypeTemplate = (appConfig, packageIds, entity) => {
    ...
    const referencedByPrivate = (Array.isArray(appConfig.apiResources) ? appConfig.apiResources : []).some(api =>
        api.entityTypeMappings?.some(mapping =>
            mapping.entityTypeTargets.some(target => target.ordId === entity.ordId && api.visibility === RESOURCE_VISIBILITY.private)
        )
    ) || (Array.isArray(appConfig.dataProducts) ? appConfig.dataProducts : []).some(dp =>
        dp.entityTypes?.some(target => target === entity.ordId && dp.visibility === RESOURCE_VISIBILITY.private)
    );

    const finalVisibility = referencedByPrivate ? RESOURCE_VISIBILITY.private : visibility;
    const packageId = packageIds.find(id => id.includes(`entityType-${finalVisibility}`));
...
const createAPIResourceTemplate = (serviceName, serviceDefinition, appConfig, packageIds, accessStrategies) => {
    ...
    const packageId = Array.isArray(packageIds)
        ? packageIds.find(id => id.includes(`api-${visibility}`))
        : null;
    ...
    paths.forEach((generatedPath) => {
        ...
        const referencedEntityTypes = Array.isArray(appConfig.entityTypes)
            ...
            : [];
        const hasPrivateEntityType = referencedEntityTypes.some(et => et.visibility === RESOURCE_VISIBILITY.private);
        const finalVisibility = hasPrivateEntityType ? RESOURCE_VISIBILITY.private : visibility;
        ...
        let obj = {
            ...
            ? packageIds.find(id => id.includes(`integrationDependency-${finalVisibility}`))
            : null,
            entityTypeMappings: referencedEntityTypes.length > 0
                ? [{ entityTypeTargets: referencedEntityTypes.map(et => ({ "ordId": et.ordId })) }]
                : undefined,
            ...ordExtensions,
        };
...
const createEventResourceTemplate = (serviceName, serviceDefinition, appConfig, packageIds, accessStrategies) => {
    ...
    const visibility = ordExtensions.visibility || RESOURCE_VISIBILITY.public;
    const packageId = Array.isArray(packageIds)
        ? packageIds.find(id => id.includes(`event-${visibility}`))
        : null;
...
const createDataProductTemplate = (dataProductName, dataProductDefinition, appConfig, packageIds) => {
    ...
    const hasPrivateEntityType = Array.isArray(appConfig.entityTypes)
        ? referencedEntityTypes.some(entityType =>
            appConfig.entityTypes.find(et => et.ordId === entityType.ordId)?.visibility === RESOURCE_VISIBILITY.private
        )
        : false;

    const finalVisibility = hasPrivateEntityType ? RESOURCE_VISIBILITY.private : visibility;
    const packageId = Array.isArray(packageIds)
        ? packageIds.find(id => id.includes(`dataProduct-${finalVisibility}`))
        : null;
