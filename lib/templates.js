...
const createEventResourceTemplate = (eventName, eventDefinition, appConfig, packageIds, accessStrategies) => {
    const ordExtensions = readORDExtensions(eventDefinition);
    const visibility = _handleVisibility(ordExtensions, eventDefinition);
    const packageId = _getPackageID(appConfig.ordNamespace, packageIds, ORD_RESOURCE_TYPE.event, visibility);
    const ordId = `${appConfig.ordNamespace}:eventResource:${eventName}:v1`;
    const entityTypeMappings = _getEntityTypeMappings(eventDefinition);
    ...
    let obj = {
        ...
        shortDescription: `${eventName} event resource`,
        description:
            eventDefinition["@description"] ??
            eventDefinition["@Core.Description"] ??
        ...
        visibility,
        resourceDefinitions: [
            _getResourceDefinition("asyncapi-v2", "json", ordId, eventName, "asyncapi2.json", accessStrategies),
        ],
        extensible: { supported: "no" },
        ...(entityTypeMappings ? { entityTypeMappings } : {}),
        ...ordExtensions,
...
function _propagateORDVisibility(model) {
    for (const [name, def] of Object.entries(model.definitions)) {
        if (def.kind === CDS_ELEMENT_KIND.service && def[ORD_EXTENSIONS_PREFIX + "visibility"]) {
            const visibility = def[ORD_EXTENSIONS_PREFIX + "visibility"];
            for (const def of model.definitions) {
                if (def.name.startsWith(name + ".")) {
                    if (!def[ORD_EXTENSIONS_PREFIX + "visibility"]) {
                        def[ORD_EXTENSIONS_PREFIX + "visibility"] = visibility;
                    }
                }
            }
        }
