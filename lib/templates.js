...
const createMCPAPIResourceTemplate = (appConfig, packageIds, accessStrategies) => {
    ...
    const packageId = _getPackageID(
        ...
        packageIds,
        ORD_RESOURCE_TYPE.api,
        RESOURCE_VISIBILITY.public,
    );
    // Fallback: ensure partOfPackage never undefined to satisfy generic apiResources tests
    const effectivePackageId = packageId || `${appConfig.ordNamespace}:package:${RESOURCE_VISIBILITY.public}:v1`;

    return {
        ordId,
        title: `MCP Server for ${appConfig.appName}`,
        shortDescription: `This is the MCP server to interact with the ${appConfig.appName}`,
        description: `This is the MCP server to interact with the ${appConfig.appName}`,
        version: "1.0.0",
        lastUpdate: appConfig.lastUpdate,
        visibility: RESOURCE_VISIBILITY.public,
