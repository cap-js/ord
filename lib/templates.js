...
const createExposedEntityTypesItemTemplate = (entity) => {
...
const createEntityTypeTemplate = (appConfig, packageIds, entity) => {
    if (entity.isODMMapping) {
        // ODM mappings are not created as entity types, they are only used in exposedEntityTypes
        return [];
    }
...
function _getExposedEntityTypes(definitionObj) {
    ...
    const entities = Object.values(definitionObj.entities).flatMap((entity) => {
        const entityData = _flattenEntityGraph(entity).map(createExposedEntityTypesItemTemplate);
