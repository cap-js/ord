 ...
 */
function generateDataProductFiles(csn, appConfig) {
    const files = [];
    ...
    for (const config of dataProducts) {
        const namespace = annotationReader.get(config.serviceName, 'namespace') 
        ...
        const dpd = buildDpdObject(baseConfig, csn, appConfig);
        files.push({
