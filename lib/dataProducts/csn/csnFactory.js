...
function buildCsnObject(config, csn, appConfig) {
            ...
            csnInitPromise = loadSchemaWithFallback('csn', cfg.schemaUrl, localPath)
                .then(res => {
                    ...
                    try {
                        ...
                        console.warn(`CSN schema logger error: ${logError.message}`);
                    }
                    return v;
                })
                .catch(error => {
                    // Schema loading failed completely
                    console.error(`Failed to initialize CSN validator: ${error.message}`);
                    return null;
                });
        } catch (configError) {
            // Configuration loading failed
            console.error(`Failed to load CSN configuration: ${configError.message}`);
        }
    }
    ...
    try { 
        validate('csn', modifiedCsn); 
    } catch (validationError) { 
        Logger.warn(`CSN schema validation failed for ${serviceName}: ${validationError.message}`); 
    }
