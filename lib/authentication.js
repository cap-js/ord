...
function createAuthConfig() {
    ...
    try {
        ...
        if (process.env.ORD_AUTH_TYPE) {
            // Handle both JSON format (legacy) and comma-separated format (GitHub Actions compatible)
            if (process.env.ORD_AUTH_TYPE.startsWith('[')) {
                // Legacy JSON format: ["mtls","basic"]
                authConfig.types = [...new Set(JSON.parse(process.env.ORD_AUTH_TYPE))];
            } else {
                // New comma-separated format: "mtls,basic"
                authConfig.types = [...new Set(process.env.ORD_AUTH_TYPE.split(',').map(t => t.trim()))];
            }
        ...
        if (
            ...
            (authConfig.types.includes(AUTHENTICATION_TYPE.Basic) ||
                authConfig.types.includes(AUTHENTICATION_TYPE.MTLS))
        ) {
        ...
        if (authConfig.types.includes(AUTHENTICATION_TYPE.Basic)) {
            ...
            } else if (process.env.BASIC_AUTH_USERNAME && process.env.BASIC_AUTH_HASH) {
                // New GitHub Actions compatible format
                credentials = {
                    [process.env.BASIC_AUTH_USERNAME]: process.env.BASIC_AUTH_HASH
                };
                Logger.log("createAuthConfig: Using GitHub Actions compatible auth format");
            } else {
                // Try new nested structure first (authentication.basic.credentials)
                if (cds.env.authentication?.basic?.credentials) {
                    credentials = cds.env.authentication.basic.credentials;
                    Logger.log("createAuthConfig: Using new nested structure authentication.basic.credentials");
                }
                // Fallback to old flat structure (authentication.credentials) for backward compatibility
                else if (cds.env.authentication?.credentials) {
                    credentials = cds.env.authentication.credentials;
                    Logger.log(
                        "createAuthConfig: Using legacy flat structure authentication.credentials (consider migrating to authentication.basic.credentials)",
                    );
                } else {
                    ...
                    try {
                        ...
                        if (cdsConfig.authentication?.basic?.credentials) {
                            credentials = cdsConfig.authentication.basic.credentials;
                            Logger.log(
                                "createAuthConfig: Loaded basic auth credentials from .cdsrc.json using new nested structure",
                            );
                        }
                        // Fallback to old flat structure
                        else if (cdsConfig.authentication?.credentials) {
                            credentials = cdsConfig.authentication.credentials;
                            Logger.log(
                                "createAuthConfig: Loaded basic auth credentials from .cdsrc.json using legacy flat structure (consider migrating to authentication.basic.credentials)",
                            );
                        }
                    } catch (error) {
            ...
            if (!credentials) {
                Logger.error("createAuthConfig:", "Basic authentication configured but no credentials found");
                return Object.assign(defaultAuthConfig, { error: "Basic authentication requires credentials" });
            }
