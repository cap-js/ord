...
`lib/ord.js` (orchestrator) -> `templates.js` (resource templates + visibility + version extraction) -> `defaults.js` (baseline values) -> `extend-ord-with-custom.js` (merge custom overrides) -> `authentication.js` (access strategies) -> `interop-csn.js` (CSN interop export) -> `ord-plugin.js` (build target wiring).
