...
const { BUILD_DEFAULT_PATH } = require("../../../lib/common/constants");

// Setup cds.build.Plugin mock BEFORE requiring ord-plugin.js
...
const OrdBuildPlugin = require("../../../lib/build/ord-plugin");
...
jest.mock("../../../lib/core/compile-metadata", () => {
    return jest.fn((url) => {
...
describe("Build", () => {
    ...
    it("should not write resources files when eventResources is empty", async () => {
        jest.spyOn(console, "log").mockImplementation(() => {});
        jest.spyOn(OrdBuildPlugin.prototype, "_generateDocument").mockImplementation(() => ({}));
        jest.spyOn(OrdBuildPlugin.prototype, "_createWorkerPool").mockImplementation(() => ({}));
        jest.spyOn(OrdBuildPlugin.prototype, "model").mockImplementation(async () => ({}));
        jest.spyOn(OrdBuildPlugin.prototype, "write").mockImplementation(() => ({ to: () => Promise.resolve() }));

        const ordBuildPlugin = new OrdBuildPlugin();

        await ordBuildPlugin.build();

        expect(ordBuildPlugin.model).toHaveBeenCalledTimes(1);
        expect(ordBuildPlugin.write).toHaveBeenCalledTimes(1);
        expect(ordBuildPlugin._createWorkerPool).toHaveBeenCalledTimes(0);
    });
    ...
    it("should throw error when metadata compilation fails", async () => {
        ...
        await expect(new OrdBuildPlugin()._generateResourcesFiles({}, resources)).rejects.toThrow();
    });
